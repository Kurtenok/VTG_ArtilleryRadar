...
/*
    ...
    Консоль оператора. Діалог, а не шар поверх екрана: оператор СИДИТЬ за
    пультом, і керування машиною на цей час йому не належить.
...
// масштаб не вгадуємо числом: ставимо будь-який, міряємо обхват по
// ширині й одним кроком перераховуємо під потрібний
...
for "_i" from 0 to (CBR_UI_ROWS - 1) do {
    ...
    // кнопка, а не клікабельний текст: клік ловиться напевно, а фон
    // прозорий, тож на пульті це той самий напис
...
// журнал і рядок стану оновлюються рідко: цифри міняються подіями
uiNamespace setVariable ["cbr_pfh", [{ [] call cbr_fnc_consoleUpdate }, 0.25] call CBA_fnc_addPerFrameHandler];
[] call cbr_fnc_consoleUpdate;

// видимість снарядів окремо, бо вона щокадрова: один промінь за кадр

...
/*
    Засічка зміщена всередині кола випадково, і зсув перекидається
    щоразу заново — між пострілами позиція гуляє по колу.

    Відхід рівномірний по РАДІУСУ, а не по площі. Рівномірність по
    площі (звідки й береться корінь) статистично чесніша, але в кільця
    площа росте з радіусом, і знаряддя тоді майже завжди опинялось
    скраю: ближче за пів радіуса — лише чверть випадків. Тут — половина,
    і центр кола стає осмисленою здогадкою.
*/
private _fnc_blur = {
    params ["_p", "_radius"];

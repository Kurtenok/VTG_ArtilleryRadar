...
/*
    ...
    Рельєф читається відліками карти висот, а не променем: на двадцять
    кілометрів той коштував би непорівнянно дорожче, а закриває станцію
    саме гора, не кущ. Поставлені об'єкти — окремою перевіркою нижче,
    карта висот про них не знає нічого.
*/
...
/*
    ...
    об'єкти не знає взагалі, тож станція з-під бетону бачила б усе небо.

    Промінь короткий навмисно. Закриває антену те, що поруч, а
    прострілювати двадцять кілометрів геометрії заради далекої будівлі
    коштувало б дорожче за всі інші перевірки разом узяті.
*/
...
/*
    ...
    швидкий снаряд може прошити зону наскрізь і впасти, а вогнева все
    одно з'явиться.
*/
...
// Як часто сервер перевіряє перелік діючих станцій про всяк випадок, с
#define CBR_MANNED_EVERY 10

...
/*
    ...
    Оператор прибирає засічку — клавішею або кнопкою в рядку журналу.
    Сам запис видаляє машина, що веде журнал; тут лише номер і звук.
*/
...
// мітку на карті не чіпаємо: вона користувацька, і прибрати її може
// будь-хто сам
...
// зі свого списку одразу: журнал приїде з мережі із затримкою, а пульт
// має відгукуватись миттєво
_log deleteAt _idx;
uiNamespace setVariable ["cbr_log", _log];
uiNamespace setVariable ["cbr_sel", _idx min (((count _log) - 1) max 0)];

// підписи лежать окремим списком у тому ж порядку — інакше поїдуть на
// один запис і підпишуть чужу засічку

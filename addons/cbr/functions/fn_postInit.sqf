...
/*
    Точка входу. Постріл ловиться там, де снаряд створено, і в спокої
    мод не коштує майже нічого: єдиний цикл — рідка перевірка на
    сервері, чи не сів хтось за пульт.
...
/*
    ...
    будь-яку зміну машини, зокрема й посадку скриптом.

    Сервер повідомляється ЗАВЖДИ, без перевірки «а чи станція це». Ця
    перевірка тут була й ламала мод у мережі: список станцій приїжджає
    до клієнта публічною змінною із запізненням, і посадка в проміжку
    здавалась посадкою в звичайну машину. Сервер не дізнавався про
    оператора, перелік діючих станцій лишався порожнім — і радар мовчав
    до кінця місії. Зайвий виклик коштує незрівнянно менше.
*/
...
if (isServer) then {
    ...
    /*
        Страховка. Подій достатньо, поки вони доходять, але в мережі
        порядок завантаження не гарантований: гравець може виявитись у
        станції ще до того, як та зареєстрована, і тоді жодної події вже
        не буде. Перерахунок дешевий — прохід по кількох машинах, — а
        мовчазна поломка коштує всієї місії.
    */
...
addMissionEventHandler ["ProjectileCreated", {
    ...
    */
...

    {
        ...
        // Сторона стрільця тут не питається: станція бачить будь-який
        // балістичний постріл, чий він — не її справа. Розбирається з
        // цим оператор, дивлячись на квадрат
